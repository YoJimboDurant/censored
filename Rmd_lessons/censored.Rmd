...
ggplot(ydata, aes(sample=x)) + stat_qq() + facet_wrap(~sample) + scale_x_continuous(breaks = -2:2, labels = 100 * signif(pnorm(-2:2, lower.tail=FALSE),2)) + theme_bw(base_size = 14, base_family = "sans") + labs(title = "Four Samples of 100 Observations From Normal Distribution", 
       caption = "4 Samples of n= 100 from Normal Distribution.",
       y = "Sample Value",
       x = "Percent Chance of Exceedence")

```

## PDFs and Random Samples

PDF's can be used to predict frequency of random samples falling within a certain range.

```{r, pdffrq, echo=FALSE, message = FALSE}
ydata <- data.frame(sample = c(rep("Sample 1",100), rep("Sample 2", 100), rep("Sample 3",100), rep("Sample 4",100)), x = rnorm(400))

ggplot(ydata, aes(x=x)) + geom_histogram(aes(y=..density..),binwidth = 0.5, alpha = 0.5) + facet_wrap(~sample) +
  xlim(-4.5,4.5) + theme_bw(base_size = 14, base_family = "sans") +
  stat_function(fun = dnorm) + 
  stat_function(fun = dnorm, 
                xlim = c(-4.5,4.5),
                geom = "line", alpha =0.5) +
...
The mean value of a distribution is equal to the sum of x time x's probability density. Since the PDF is approaches zero for any single value, we have to use calculus to represent this formally:

$$
\mu = \int_{-\infty}^\infty f(x) dx
$$

...
## Calculating a Sample Mean from a ECDF (1)
...
## Calculating a Sample Mean from a ECDF (2)

```{r coffee, echo=FALSE}
mutate(dfx, `Contrib` = max(`Dollar Contribution`) + 1 - `Dollar Contribution`, `Dollar Contribution` = `Dollar Contribution`-1) %>%
  ggplot(aes(x = `Dollar Contribution`, y=Contrib)) + geom_step() + scale_y_continuous(breaks = 1:10) +
  scale_x_continuous(breaks = 0:10) + 
  labs(y = "Number Willing to Give \n(at least)",
       x = "Dollar Amount") + theme_bw() +
  annotate("text", x = 1:10, y =10:1 * 0.5, label = paste(1:10, "x", 10:1))
```

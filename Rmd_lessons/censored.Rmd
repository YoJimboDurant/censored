...
```{r coffee, echo=FALSE}
dfx <- data.frame(Name = c("Brian", "Mike", "Amy", "Susan", "Rick", "Charles", "Rose", "Elaine", "Sharon", "John"),
...
ggplot(xdf, aes(x=`Dollar Contribution`, y=`Contrib`)) + geom_step() + scale_y_continuous(breaks = 1:10) +
...
ggplot(xdf, aes(x=`Dollar Contribution`, y=`Contrib`/10)) + geom_step() + scale_y_continuous(breaks = 1:10/10) +
  scale_x_continuous(breaks = 0:10) + 
  labs(y = "S(x)",
       x = "Dollar Amount") + theme_bw() +
...
The probability of exceeding a given value of $x$, is the all of the lower values.

Assuming Charles gives us an answer of, " I will give you no more than 6 dollars!" If we assume that Charles would give any amount between 0 and 6 with equal probabilty, we could assign that extra probability to the observations below 6 
